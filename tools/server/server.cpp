...
int main(int argc, char ** argv, char ** envp) {
    ...
    if (is_router_server) {
        ...
        const auto azure_proxy = [&models_routes](const server_http_req & req) {
            server_http_req req_override = req;
            try {
                json body = json::parse(req.body);
                body["model"] = req.get_param("deployment_id");
                req_override.body = body.dump();
            } catch (const std::exception &) {
                // let the proxy handle parsing/validation errors
            }
            return models_routes->proxy_post(req_override);
        };
        ...
        routes.post_completions_azure      = azure_proxy;
        routes.post_chat_completions       = models_routes->proxy_post;
        routes.post_chat_completions_azure = azure_proxy;
        routes.post_anthropic_messages     = models_routes->proxy_post;
        routes.post_anthropic_count_tokens = models_routes->proxy_post;
        routes.post_infill                 = models_routes->proxy_post;
        routes.post_embeddings             = models_routes->proxy_post;
        routes.post_embeddings_oai         = models_routes->proxy_post;
        routes.post_embeddings_azure       = azure_proxy;
        ...
        routes.get_azure_deployments = models_routes->get_router_azure_deployments;
    ...
    ctx_http.get ("/openai/deployments",                                ex_wrapper(routes.get_azure_deployments));
    ctx_http.post("/openai/deployments/:deployment_id/completions",     ex_wrapper(routes.post_completions_azure));
    ctx_http.post("/openai/deployments/:deployment_id/chat/completions", ex_wrapper(routes.post_chat_completions_azure));
    ctx_http.post("/openai/deployments/:deployment_id/embeddings",      ex_wrapper(routes.post_embeddings_azure));
