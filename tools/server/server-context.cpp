...
static std::unique_ptr<server_res_generator> handle_completions_impl(
    ...
    try {
        ...
        const std::string oaicompat_model = json_value(data, "model", ctx_server.model_name);
        for (size_t i = 0; i < inputs.size(); i++) {
            ...
            task.params.oaicompat_model   = oaicompat_model;
...
void server_routes::init_routes() {
    ...
    this->post_completions_azure = [this](const server_http_req & req) {
        server_http_req req_override = req;
        try {
            json body = json::parse(req.body);
            body["model"] = req.get_param("deployment_id", ctx_server.model_name);
            req_override.body = body.dump();
        } catch (const std::exception &) {
            // let the downstream handler surface parsing errors
        }
        return this->post_completions_oai(req_override);
    };
    ...
    this->post_chat_completions_azure = [this](const server_http_req & req) {
        server_http_req req_override = req;
        try {
            json body = json::parse(req.body);
            body["model"] = req.get_param("deployment_id", ctx_server.model_name);
        ...
        return this->post_chat_completions(req_override);
    };
    ...
    this->get_azure_deployments = [this](const server_http_req &) {
        auto res = std::make_unique<server_res_generator>(ctx_server);
        json model_meta = nullptr;
        if (is_ready()) {
            model_meta = ctx_server.model_meta();
        ...
        json deployments = {
            ...
            {"data", json::array({
                json{
                    ...
                    {"model", deployment_id},
                    {"owner", "llamacpp"},
                    {"status", is_ready() ? "succeeded" : "loading"},
                    {"meta", model_meta},
                }
            })}
        };
    ...
    this->post_embeddings_azure = [this](const server_http_req & req) {
        server_http_req req_override = req;
        try {
            json body = json::parse(req.body);
            body["model"] = req.get_param("deployment_id", ctx_server.model_name);
        ...
        return this->post_embeddings_oai(req_override);
    };
...
std::unique_ptr<server_res_generator> server_routes::handle_embeddings_impl(const server_http_req & req, task_response_type res_type) {
    ...
    // write JSON response
    const std::string model_name = json_value(body, "model", ctx_server.model_name);
    json root = res_type == TASK_RESPONSE_TYPE_OAI_EMBD
        ? format_embeddings_response_oaicompat(body, model_name, responses, use_base64)
        : json(responses);
    res->ok(root);
    return res;
}
